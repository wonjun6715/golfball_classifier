...
class CNN_Ball(nn.Module):
    def __init__(self): # 클래스 초기화 함수
        ...
        self.fc1=nn.Linear(1025*8192,4) # 레이어의 입력 크기 1025*8192, 출력 크기 4
...
while True: 
    ...
    try:
        ...
        if 'STEP' in dict_json and dict_json['STEP'] == 1: # 데이터 안에 'STEP' 키가 존재하고 값이 1인 경우에 실행
            ...
            test_file_name = "./sound/TEST/TEST_{}.wav".format(Position) # 테스트할 오디오 파일명 설정
            test_wav=readFile(test_file_name) # 오디오 파일을 읽고 전처리
            outputs = test_net(torch.tensor(test_wav).reshape(-1,1025*8192)) # 이미지 사이즈는 1025 * 8192
